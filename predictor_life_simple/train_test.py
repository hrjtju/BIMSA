...
def plot_scalar(scalar_dict: dict, base_dir: str) -> None:
    ...
    ax1.plot(pd.Series(scalar_dict["train_loss"]).rolling(window=len(scalar_dict["train_loss"])//10, min_periods=1, center=True).mean(), label="train_loss (smoothed)")
    ax1.legend()
    ax1.grid()
    ax1.semilogy()
    ...
    ax2.plot(pd.Series(scalar_dict["grad_norm"]).rolling(window=len(scalar_dict["grad_norm"])//10, min_periods=1, center=True).mean(), label="grad_norm (smoothed)")
    ax2.legend()
    ax2.semilogy()
    ax2.grid()
    ...
    ax3.plot(pd.Series(scalar_dict["train_acc"]).rolling(window=len(scalar_dict["train_acc"])//10, min_periods=1, center=True).mean(), label="train_acc (smoothed)")
    ax3.legend()
    ax3.grid()
